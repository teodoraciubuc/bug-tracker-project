...
router
    .post('/', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
    ...
    .post('/:id/join', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
    ...
    .patch('/:id', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, description, repoUrl } = req.body;
        const userId = req.user.id;
    ...
    .delete('/:id', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const project = await prisma.project.findUnique({
