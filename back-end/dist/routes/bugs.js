...
router
    .post('/projects/:projectId/bugs', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
    ...
    .get('/projects/:projectId/bugs', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { projectId } = req.params;
        ...
        const bugs = await prisma.bug.findMany({
            where: status ? { projectId, status: status } : { projectId },
    ...
    // ASSIGN BUG -> doar MP 
    .patch('/:bugId/assign', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
    ...
    .patch('/:bugId/status', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { bugId } = req.params;
    ...
    .patch('/:bugId', authMiddlewares_1.requireAuth, async (req, res, next) => {
    try {
        const { bugId } = req.params;
        const { title, description, severity, priority } = req.body;
        const userId = req.user.id;
